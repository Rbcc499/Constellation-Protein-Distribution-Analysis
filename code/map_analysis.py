...
Analysis logic for Constellation:
...
- CSV should follow the template: Object1, Object2, SpotIndex1, ClosestSpotIndex2, MinDistance (plus optional MeanDistance/MaxDistance).
"""
...
#loads distance table
def load_spots_distances(path: str) -> pd.DataFrame:
    #get file type (.csv or .xlsx), convert to lower case
    ext = os.path.splitext(path)[1].lower()

    #can load either .csv or .xlsx, error if other file type
    if ext == ".csv":
    ...
    #check that Object1 AND Object are present
    ...
    #check that MinDistance is present
    if "MinDistance" not in df.columns:
        raise ValueError('Data file must contain "MinDistance" column. Please refer to template')

    #drop other distance columns
...
#convert/clean Imaris object names, get first protein name
def clean_protein_name(raw: str) -> str:
    #check for blank cells (float/NaN) and return empty string to remove
    if raw is None or (isinstance(raw, float) and math.isnan(raw)):  #type: ignore[arg-type]
        return ""
    #replaces _ to empty space
    s = str(raw).replace("_", " ").strip()
    if not s:
        return ""
    return s.split()[0]

#filename parser, file name should be structured: mouseid_slice_roundno, see readme for examples
def parse_mouse_slice_round(filename: str) -> Tuple[str, str, int]:
    ...
    mouse = parts[0]
    slice_id = parts[1]
    #replace digits with "", convert to int
    round_id = int(re.sub(r"\D", "", parts[2]))
...
#get save path, will save alternate if current is open
...
def get_protein_pairs_with_distances(data_folder: str):
    ...
    #get list of protein names, and pairs
...
#clean df table proteins for analysis
...
#load the expansion factor excel and clean
...
def normalize_reference_pairs(reference_pair: Optional[str]) -> List[str]:
    ...
    parts = [p.strip() for p in re.split(r"\s+(?:OR|AND)\s+", s, flags=re.IGNORECASE) if p.strip()]
...
def apply_alignment_filter(
    ...
    # Discover proteins present (MUST include Object2_df* as well)
    obj1_cols = [c for c in df_all.columns if c.startswith("Object1_df")]
    obj2_cols = [c for c in df_all.columns if c.startswith("Object2_df")]
    obj_cols = obj1_cols + obj2_cols
    ...
    # Candidates: exclude only the endpoints; allow same-location proteins (needed for PSD95 + BC262 cases)
    candidates = []
    for p in proteins_in_df:
        if p in (A, C):
            continue
        locp = protein_locations.get(p, "")
        if locp:  # must have a compartment set
            candidates.append(p)
...
def run_analysis(
    ...
    *,
    ...
    for csv_name in data_files:
        ...
        #alignment filtered (toggled ON/OFF)
        if use_alignment_analysis:
            ...
            # AND across all selected reference pairs:
            for rp in ref_pairs:
                df_filtered = apply_alignment_filter(
                    df_filtered,
                    reference_pair=rp,
            ...
            if not df_filtered.empty:
                df_filtered["Reference_Pair"] = " AND ".join(ref_pairs)
